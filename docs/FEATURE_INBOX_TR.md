...
| F-007 | Şifreli günlük (journal) modu | in_progress | yüksek | M | Faz 2 | Journal workspace + app lock/passcode + encryption service + Keychain/LocalAuthentication adaptörleri eklendi; lock sırasında editor state privacy reset + locked editor placeholder + kayıtta "Encrypt On Save" + read-only secure preview/decrypt akışı eklendi; production wiring ve decrypt UX polish sırada. |
| F-008 | AI ile otomatik günlük modları üretimi | in_progress | orta | L | Faz 3 | ContextSuggestionService Journal workspace'e bağlandı (Context Mode ile tek adım başlangıç); mood + daypart bazlı psikolojik odaklı JourneyTemplateService (otomatik şablon taslağı) eklendi; gerçek AI provider + kişisel ton uyarlaması sırada. |
...
| F-024 | Elk-benzeri sade hedef ülke kur görünümü | in_progress | yüksek | M | Faz 3 | Frankfurter provider + tek kur kartı + live/cached + offline fallback + dakika bazlı güncellik (updated Xm) + 60 dk stale göstergesi eklendi; UX polish sırada. |
...
| F-038 | Trip yaklaşma bildirimleri (T-3 gün / T-24 saat) | in_progress | yüksek | M | Faz 3 | TripAlertService ile pre-trip check bildirim planlama eklendi; OS-level notification category ve kullanıcı ayar ekranı sırada. |
| F-039 | Journey place review (lokasyon etiketi + yıldız + yorum) | in_progress | yüksek | M | Faz 3 | JourneyEntry/PlaceReview domain + JourneyService + locationTag bazlı sorgu eklendi; UI katmanında tam review kartı ve harita entegrasyonu sırada. |
| F-040 | İç sosyal akış (post/comment/like-dislike/follow) | in_progress | yüksek | L | Faz 3+ | Social domain + repository + SocialFeedService ile temel feed algoritması eklendi; moderation, privacy ve realtime dağıtım sırada. |
| F-041 | Unique username + avatar profil | in_progress | yüksek | M | Faz 2-3 | UserProfile modeli + unique username kontrolü + avatar path desteği + UserProfileService eklendi; auth/session UI bağlama sırada. |
| F-042 | Rozet sistemi (seyahat + paylaşım) | in_progress | orta | M | Faz 3+ | 4 rozet tanımı (Gezgin Başlangıç, Ülke Koleksiyoncusu, Hikaye Anlatıcısı, Topluluk Favorisi) + BadgeService kriter hesaplama eklendi; profil UI ve seviye progression sırada. |
| F-043 | Journal Archive / Timeline (ay bazlı hafıza kartları) | in_progress | yüksek | M | Faz 2-3 | JournalArchiveService + JournalArchive view/viewmodel eklendi; mood filtre, encrypted-safe excerpt ve note detail drill-down polish sırada. |
